...
namespace Practical.AI.Agents.GUI
{
    public partial class MarsWorld : Form
    {
            ...
            for(var i = 0; i < _n; i++)
            {
                for(var j = 0; j < _m; j++)
                {
                    ...
                    foreach(var belief in _marsRover.Beliefs)
                    {
                        ...
                        if(pred != null && !pred.Contains(new Tuple<int, int>(i, j))) { continue; }
        ...
        private void TimerAgentTick(object sender, EventArgs e)
        {
            ...
            if(beliefs != beliefsList.Text) { beliefsList.Text = beliefs; }
            if(desires != desiresList.Text) { desiresList.Text = desires; }
            if(intentions != intentionsList.Text) { intentionsList.Text = intentions; }
        ...
        private string UpdateText(RichTextBox list, IEnumerable<object> elems)
        {
            ...
            foreach(var elem in elems) { result += elem; }
        ...
        {
            foreach(var b in _marsRover.Beliefs) { beliefsList.Text += b; }
            foreach(var d in _marsRover.Desires) { desiresList.Text += d; }
            foreach(var i in _marsRover.Intentions) { intentionsList.Text += i; }
        }
    }
}
